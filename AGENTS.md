...
- **It is a hard dependent of `lnd`.** `main.ts` resolves LND's gRPC endpoint over the LXC bridge through the `bridgeAddress` helper in `startos/utils.ts` (reading `lnd`'s `gRPCHostId` binding's assigned external port, with `gRPCHostId` / `gRPCPort` imported from `lnd-startos/startos/interfaces`) and pins that `host:port` into Jet's `api/config.json` (`serverAddress`). The `.const()` re-resolves reactively: while LND is absent or still locked it pins the `127.0.0.1:10009` loopback placeholder, then heals with one restart once the gRPC binding appears. LND's admin macaroon and `tls.cert` are read off a read-only dependency mount at `/mnt/lnd`.
