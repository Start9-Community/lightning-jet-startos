...
This repo packages [Lightning Jet](https://github.com/itsneski/lightning-jet) for StartOS.
...
```bash
git submodule update --init --recursive    # initialize the lightning-jet submodule
npm ci                                     # install dependencies
make                                       # build the universal .s9pk
```

The `main` image is built from the `lightning-jet/` git submodule — it must be checked out before `make`.

## Updating the upstream version

Lightning Jet is built from the `lightning-jet/` git submodule. There is no `dockerTag` in the manifest.
